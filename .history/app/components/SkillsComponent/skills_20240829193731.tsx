...
export default function Skills() {
  return (
    <div className="flex flex-col items-center justify-center gap-10 font-josefin text-5xl font-bold text-emph-dark dark:text-white lg:w-screen lg:text-6xl">
      ...
      <div className="grid grid-flow-col grid-rows-3 gap-20 rounded-xl">
        {skillsLogo.map((skills, index) => (
          <motion.li
            key={index}
            className="grid w-16 lg:h-28 lg:w-28 place-items-center rounded-xl bg-emph-dark"
            variants={slideIn}
            initial="initial"
            whileInView="animate"
            viewport={{ once: true }}
          ...
          >
            <Image
              ...
              src={skills.src}
              alt="Skills Logo"
              className="lg:w-12 bg-emph-dark"
            />
          </motion.li>
        ))}
      </div>
